...
finProg:
	...
	ret ;esta ret corresponde a la de finalizar el programa
...
;cerrarArchivo:    
;        mov     rcx,qword[handlo] ;# OJO dejar solo handle..?
;        sub     rsp,32
;        call    fclose
;        add     rsp,32
;ret
...
llenarMatriz:
    ...
    leerRegistro:
            ...
            mov     byte[matriz + ebx],1    ;meto un 1 en la matriz x consifna
...
VALREG:
    ...
    validarEmpresa:
        ...
        ;#conteoDias
        mov		byte[datoValido],"N"	;Paso el dia en binario a una variable [datoBin]
...
informe:
    ...
    ;___Validacion por RANGO ingreso Datos Usuario___
        cmp		dword[CodigoEmpNum],1  ;minimo
        jl		ingresoDatosUsuario
        cmp		dword[CodigoEmpNum],6  ;maximo
    ...
    ;ya tengo el nro ingresado [1..6] en binario (double word 4 bytes)
        sub		dword[CodigoEmpNum],1 ;desplaz. filas (fila - 1)
